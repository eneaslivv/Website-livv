...
export function Sidebar({ onNavigate, activeView = "clips" }: SidebarProps) {
  ...
  return (
    <aside className="hidden md:flex fixed left-0 top-0 w-[220px] h-screen backdrop-blur-2xl bg-[#f0e9e1]/90 flex-col z-50 border-r border-[#ddd5cc]">
      <div className="px-3 py-1.5">
        <button className="flex items-center gap-2 text-[#8a7e74] hover:text-[#2c2420]/70 text-[12px] transition-all duration-200 px-1.5 py-1 rounded-md hover:bg-[#2c2420]/[0.04]">
      ...
      <div className="px-3 pt-4 pb-1">
        <div className="flex items-center justify-between mb-1.5 px-1.5">
          <span className="text-[#8a7e74] text-[10px] font-semibold uppercase tracking-widest">Workspaces</span>
          <button className="text-[#8a7e74] hover:text-[#2c2420]/70 text-[11px] font-medium transition-colors duration-200">
            + Add
          </button>
        </div>
        <button
          onClick={() => setWorkspaceOpen(!workspaceOpen)}
          className="flex items-center gap-2 w-full hover:bg-[#2c2420]/[0.05] rounded-lg py-1.5 px-1.5 transition-all duration-200"
        >
          <div className="w-5 h-5 rounded-md bg-gradient-to-br from-[#ddd5cc] to-[#c9a48a] flex items-center justify-center text-[#2c2420]/60 text-[10px] font-semibold shadow-sm">
            D
          </div>
          <span className="flex-1 text-left text-[#2c2420]/80 text-[13px] font-medium">Default</span>
          <svg
            className={`w-3.5 h-3.5 text-[#8a7e74] transition-transform duration-200 ${workspaceOpen ? "rotate-0" : "-rotate-90"}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2.5 w-full px-2 py-1.5 rounded-md text-[13px] font-medium transition-all duration-200 ${
                    isActive
                      ? "bg-[#2c2420]/[0.06] text-[#2c2420] shadow-sm"
                      : "text-[#8a7e74] hover:text-[#2c2420]/80 hover:bg-[#2c2420]/[0.04]"
                  }`}
                >
                  <span className={isActive ? "text-[#2c2420]" : "text-[#8a7e74]"}>
      ...
      <div className="px-4 py-3 mt-auto border-t border-[#ddd5cc]">
        <div className="flex flex-wrap gap-x-3 gap-y-1.5">
          {["Pricing", "Help Center", "Changelog", "Terms", "About"].map((link) => (
            <button
              key={link}
              className="text-[#8a7e74]/60 hover:text-[#8a7e74] text-[10px] font-medium transition-colors duration-200"
            >
              {link}
            </button>
          ))}
